...
RSpec.describe ShoppingCart::CreditCard, type: :model do
  ...
  describe '.years' do
    it 'return array of integers' do
      all = ShoppingCart::CreditCard.years.all?(&is_int)
      expect(all).to eq true
    end
    it 'return years from now' do
      now = Time.now.strftime('%y').to_i
      expect(ShoppingCart::CreditCard.years.min).to eq now
    end
  end

  describe '.month' do
    it 'return array of integers' do
      all = ShoppingCart::CreditCard.months.all?(&is_int)
      expect(all).to eq true
    end
    it 'return number of month' do
      expect(ShoppingCart::CreditCard.months).to eq (1..12).to_a
    end
  end
end
